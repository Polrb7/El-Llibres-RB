...
class ProfileController extends Controller
{
    ...
    public function update(ProfileUpdateRequest $request): RedirectResponse
    {
        ...
        if ($request->hasFile('profile_img')) {
            ...
            $extension = $request->file('profile_img')->getClientOriginalExtension(); //recuperem la extensio
            $imageName = $user->id . '.' . $extension; //nom imatge = id.extensio
            $path = $request->file('profile_img')->storeAs('profile_images', $imageName, 'public'); //creem el path
            // $path = File::p
            $user->profile_img = $path; //actualitzem el camp imatge de l'usuari per preparar-lo per la BD
